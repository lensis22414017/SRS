export const meta = {
  name: 'oponly-extract-batch2',
  description: 'Extract OP-only sampling data from 200 papers (batch2)',
...
phase('Extract')
// Args passed by Workflow tool directly
const P = typeof args === 'string' ? JSON.parse(args) : (Array.isArray(args) ? args : [])
log('Processing ' + P.length + ' OP-only papers (batch2)')
const r = await pipeline(P, x => agent(
  ...
  'Reject: stat rows(Mean/SD/Max/Min/Median/CV/Range), thresholds(Grade/GB15618/GB36600/Dutch optimum/action), risk indices(HQ/TEQ/Nemerow/RI/Igeo/pollution index), pot/greenhouse/spiked experiments, water, air, plant tissue, remediation effects. ' +
