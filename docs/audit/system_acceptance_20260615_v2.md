...
> 本轮在 Codex v1（`system_acceptance_20260615.md`）主干闭环基础上，**补齐裴总指出的体验级/可用性缺口**，并完成系统级复验。
...
- **如实标注**：RAG 实质为关键词检索（无向量库）；模型 AUC≈1.0 为阈值派生虚高风险；天地图底图仍受 key 白名单限制；`.app/.pkg` 仍未达正式安装包标准。
...
**改动**（方案：用 matplotlib 离线画采样点散点，**不依赖天地图 key**，符合"数据真实性 > UI"）：
...
- 新增 `run_preflight`：检测端口占用 / SQLite 目录可写 / Redis 可达性 / AI key 配置态 / 天地图 key 配置态。**只读探测，不改任何配置**。
- `_preflight_summary`：渲染为可读文本（✅/⚠️/❌ 图标）。
- `main`：启动前跑自检并打印摘要；有阻断项时弹原生提示框（macOS rumps/tkinter）。
- 托盘新增"环境自检"菜单项（可随时重看，重新跑实时自检）。
- **不动 onefile/onedir 重构**（裴总选"后续"）。

**验收**：
- 场景 1（端口 18099 空闲）：端口✅、数据库✅、Redis✅、AI key✅（脱敏 `sk-akl***oaod`）、天地图⚠️→"0 阻断 1 警告，可正常启动"。
...
1. **天地图底图**：前端 key 直连返回"IP不匹配"；后端 `TIANDITU_KEY` 为空时瓦片代理正确返回 503。需在天地图控制台配置出口 IP/域名白名单。**报告内静态图件不依赖天地图 key，已解决报告层地图可视化**。
...
- v2 补齐裴总指出的"能用、好看、完整"层：EDA 多图、批量导入、附件下载、报告地图真图、打包自检、RAG 容错，并逐项 API/产物级复验。
- 测试数：47 → 68 passed。
...
### 背景
v1 地图依赖天地图在线瓦片，受 key 白名单（IP/Referer）限制，桌面打包（exe/dmg）分发 + 内网场景下走不通（IP 不固定、key 会泄露、需外网）。本轮重构为**三层离线地图架构**。
...
- **L2 下载**：`scripts/download_tianditu_mbtiles.py`（按 adcode/bbox 下载天地图影像到 MBTiles，支持断点续传、体积预估）。
...
- ✅ **天地图白名单问题彻底解决**：L1 不依赖天地图；L2 在线仅服务器固定 IP 场景用。
- ⚠️ 乡镇级边界：DataV 开放数据最细到县级，乡镇级无免费开放数据源；县级粒度匹配土壤修复业务（按市县监管审批）。

详细配置见 `docs/地图离线方案与天地图配置.md`。

## 七、最终残留风险（如实标注）

1. **天地图 L2 在线**：仅服务器固定 IP 场景适用；桌面打包走 L1/L2-MBTiles。
