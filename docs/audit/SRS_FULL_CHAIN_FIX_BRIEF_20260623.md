...
适用项目: `/Users/lensis/Claude/Projects/SRS`
...
```bash
cd /Users/lensis/Claude/Projects/SRS/backend
.venv/bin/pytest -q
```
...
```text
你是 GLM5.2，启用 ultracode。任务是在 /Users/lensis/Claude/Projects/SRS 中修复污染场地监管系统核心 bug，目标是让真实数据导入、导出、EDA、SSUI、推荐/技术库、AI、地图与审计闭环真实可用。
