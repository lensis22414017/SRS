...
因此: 完整 pytest、`npm run build`、Docker 流程、AI 模型实调、浏览器截图**均需裴总本机执行**。
...
- **方案A(推荐): 独立仓库**。`cd /Users/lensis/Claude/Projects/SRS && git init && git add -A && git commit -m "feat: SRS MVP + 验证体系"`。`.gitignore` 已覆盖 .env/db/storage/node_modules/dist/生成态数据。
- **方案B: 父仓库内 pathspec 暂存**。`git -C /Users/lensis add -- 'Claude/Projects/SRS'` 并确认 `git status` 不含 `backend/.env`、`frontend/.env.local`、`*.db`、`storage/`、`node_modules/`、大 CSV。
