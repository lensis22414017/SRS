...
- 地图采样点图层可用；天地图底图未通过，原因是当前 key 返回 `IP不匹配`，需要在天地图控制台修正 IP/域名白名单。
- Docker compose 已补传 AI 与天地图相关环境变量；真实 `deploy/.env` 仍使用 SiliconFlow/Qwen，代码默认与 `.env.example` 为 GLM。
...
- `/api/v1/map/tile/img/1/0/0`
  ...
  - 前端 `.env.local` key 直连天地图返回 403：`IP不匹配`。
...
2. Docker compose 未向后端传 AI/天地图环境变量。
...
1. 天地图底图未通过：当前 key 返回 `IP不匹配`，需在天地图控制台配置本机/Docker 出口 IP 或使用后端 `TIANDITU_KEY`。
