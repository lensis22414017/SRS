...
- **最后提交**：`0dccc9a` 2026-06-13 23:29 `feat: 新增地图模块 (天地图瓦片代理 + map router)`
- **2026-06-14/15 无新提交**
...
- 天地图瓦片需配置 `TIANDITU_KEY`（`.env` 中补充），否则地图降级 OSM，截图时注明。
...
# [MODE: EXECUTE] 辛特助执行记录 — 2026-06-15（晚间）

> 承 Codex v1 验收（`system_acceptance_20260615.md` 主干闭环已通），本轮针对裴总指出的"能用/好看/完整"层缺口做补齐 + 系统级复验。详见 `system_acceptance_20260615_v2.md`。
...
4. **报告地图图件**：`report_service.py` 用 matplotlib 离线画采样点散点（按超标倍数着色，不依赖天地图 key），注入 `map_summary.map_image`；HTML 模板 + DOCX 均嵌入图。`requirements.txt` 加 matplotlib。
5. **打包首启自检**：`launcher.py` 加 `run_preflight`（端口/DB/Redis/AI key/天地图 key 只读探测）+ 托盘"环境自检"项 + 阻断提示框。两场景验收通过。
...
## 残留风险（如实标注，未回避）
- 天地图底图仍受 key 白名单限制（报告内静态图件已不依赖，解决报告层）。
...
**下一步建议**：裴总若要提交，可发指令让我 `git add -A && commit`（当前未提交，遵循"不主动 commit"）。

---

## 追加：地图离线方案（2026-06-15 夜间）

裴总指出最终交付是 exe/dmg 安装包 + 内网环境，天地图在线方案（IP 白名单）桌面分发走不通。重构为三层离线地图：

- **L1 矢量底图（默认，27MB）**：全国 35 省/475 地市/2728 县行政区边界，DataV 开放数据，**完全离线，无 key/无外网**，开箱即用。`SiteMap.tsx` 三级金字塔懒加载（省→地市→县随缩放下钻）。
- **L2 MBTiles（可选）**：`scripts/download_tianditu_mbtiles.py` 按区域下载天地图影像，桌面版按需导入。
- **L2 在线（可选）**：仅服务器固定 IP 场景，配 TIANDITU_KEY + IP 白名单。

数据：`data/geo/` 27MB 已下载齐全。后端 `/map/geo/index`+`/map/geo/boundaries` 三级 + `/map/tile` 优先 MBTiles。打包 spec 加 data/geo + matplotlib（修复误排除）。详细见 `docs/地图离线方案与天地图配置.md`。

验收：地图测试 5 passed（含个旧市定位），后端全量 70 passed，前端 build EXIT=0。乡镇级无免费开放数据源，县级匹配业务粒度。

**天地图白名单结论**：桌面打包版用 L1 不需要白名单；服务器固定 IP 才用 L2 在线 + IP 白名单（填服务器出口公网 IP）。
