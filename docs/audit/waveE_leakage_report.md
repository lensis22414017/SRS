...
> 生成：2026-06-25 | 模式：EXECUTE（裴总拍板路径 C）| 证据：16 模型 meta.json + ΔAUC 量化
> 依据：Hu et al. 2026 *Commun Earth Environ* 7:214（标签泄漏专论）+ plan Wave E 修正（裴总 2026-06-24 科研深问）

## Executive Summary

按裴总 2026-06-25 拍板的**路径 C（双版本对照）**，同时训练含浓度版（full，标签泄漏组）与 X_barrier 纯协变量版（barrier，防泄漏组），以 AUC 差距量化标签泄漏。
