...
> 当前训练数据 `data/raw/模拟特征表_F127_n11690.csv`（11690×136，F1-F127 中文物理特征）是**模拟特征表，非真实文献数据**。此前文件名为"真实数据集.csv"且版本标"真实_n1119"属命名误导，已正本清源。模型 `is_real_data: False`，AUC=1.0 为模拟数据虚高（标签由特征确定性派生），**不可外推真实场地**。

- 训练样机（当前·模拟）:`data/raw/模拟特征表_F127_n11690.csv`（11690 样本，F1-F127 模拟物理特征，`is_real_data: False`）。训练前剔除 ID/污染风险等级/土地利用类型/经纬度等唯一标识与标签派生列（防泄漏，见 `dropped_leakage_cols`）。
- 训练样机（真实·待切换）:`data/raw/merged_std33,zh .xlsx`（41504×719，真实文献数据，带 DOI/Source/Year/Journal，含重金属 Cr/Hg/As/Pb/Cu/Zn/Ni/Cd + 有机物 + 理化肥力）。后续重建模型时切换，预期 AUC 0.7-0.9。
- 推理输入:某场地 `measurements` 长表透视为样本×因子矩阵。
- 样机参考:`参考文件/10.SHAP机器学习分析.ipynb`(不得直接复制为生产代码,须拆为可测模块)。
- 方法依据:年度报告(RF+SHAP 为主线;虚拟数据集 AUC≥0.996 为模拟参考,不可当真实性能)。
...
- **特征工程**(`ml/models/data_prep.py`):剔除 ID/污染风险等级/土地利用类型/经纬度等唯一标识与标签派生列(防泄漏);剔除缺失率>95% 列;其余中位数填充 + `*__missing` 标记列;数据版本 `模拟特征表_F127_n11690`(`is_real_data: False`,AUC=1.0 为模拟虚高)。
