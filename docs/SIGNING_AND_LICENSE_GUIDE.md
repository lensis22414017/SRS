# 项目组发布前的两项指导：代码签名证书 + WeasyPrint/GPL/MIT 协议

> 项目组整理于 2026-06-30。软件全部没问题后申请软著(项目组已定), 此处只讲另两项。
...
**步骤 4：用证书签名 exe**（项目组指导项目组执行，约 5 分钟）
收到 .pfx 后, 在 PowerShell 跑(项目组只需提供 pfx 路径+密码):
```powershell
# 1. 装 SDK 工具(若没有)
...
- 项目组只需: 选机构 + 提交材料 + 收 .pfx, 签名操作项目组代劳

---

## 二、WeasyPrint GPL 边界 + MIT 协议指导

### 2.1 事实查证（项目组不假设，基于包元数据）
...
- 若项目组想完全无 GPL/LGPL 边界(极致保守)→ 方案 B, 项目组改 report_service 降级链(30 分钟)
...
| 事项 | 项目组建议 | 项目组需做什么 |
|---|---|---|
| 软著 | 软件全部没问题后申请 | 后续准备材料, 项目组可帮整理源代码鉴别文档 |
| 代码签名 | 办理 OV 或 EV 证书 | 选机构 + 提交材料 + 收 .pfx, 签名操作项目组代劳 |
| WeasyPrint/GPL | 方案 A(LGPL 声明, 已在 NOTICE) | 无需操作, 项目组确认即可; 或选方案 B 项目组改代码 |
| MIT 协议 | 保持 MIT(已创建 LICENSE) | 无需操作 |

项目组确认方案 A / B 后, 项目组执行(若 B 则改降级链, 若 A 则不需要改代码)。代码签名等项目组拿到 .pfx 项目组指导签。
