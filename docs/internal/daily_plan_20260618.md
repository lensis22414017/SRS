...
> 项目组自动巡检生成 | [MODE: PLAN] | 项目组确认后可发送 ENTER EXECUTE MODE 执行
...
| 桌面打包（.app/.dmg） | ✅ 完成（需本机重建） | 黑屏/404/黑图标三项已修复，项目组需本机 `pyinstaller` |
...
1. **打包未本机重建**：黑屏修复代码已入库（`230e8ad`），但 `dist/SRS.app` 是旧版本，项目组需本机执行 `pyinstaller packaging/srs.spec --clean` 生成新 .app，否则演示仍黑屏。
