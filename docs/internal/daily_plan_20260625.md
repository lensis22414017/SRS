...
**项目组** 巡检报告 | 日期：2026-06-25 | 当前模式：PLAN
...
| 深度诊断报告 | `项目组11问题深度诊断与改进规划_20260624.md` 逐条验证、根因定位 |
...
1. **AI 连通性真实修复**：区分 `has_config` vs `connectivity_ok`；修复 OpenAI 模型名不匹配（`大语言模型-MLX-4bit` ≠ SiliconFlow 服务端模型名）；UI 不得显示"已配置=可用"。
