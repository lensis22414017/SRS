# SRS API 验收报告(项目组 2026-06-25)

> **UI 截图环境声明**: macOS 对 playwright 自带浏览器二进制 SIGKILL + chrome 单例资源耗尽,
> 项目组穷尽 11 路技术路径(Playwright/puppeteer/chrome-devtools MCP、Python playwright chromium/webkit、
...
## UI 截图采集指引(需项目组手动, 100% 可靠)

项目组在 Claude Code 输入框输入:
```
! open http://localhost:5173
```
用项目组签名的系统 chrome(GUI)打开 → 绕过 macOS SIGKILL → 手动 Cmd+Shift+3/4 截 14 张。
