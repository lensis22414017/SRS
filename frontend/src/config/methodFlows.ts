...
export const METHOD_FLOWS: Record<string, FlowConfig> = {
  obstacle_analysis: {
    ...
    title: "障碍因子分析",
    subtitle: "KOS 关键障碍因子综合评分（规则主导 + 模型辅助）",
    svgPath: svg("obstacle_analysis"),
    inputs: [
      { label: "场地检测数据", desc: "采样点污染物浓度、土壤理化性质等长表数据，来自场地导入模块" },
      { label: "GEE 空间协变量", desc: "NDVI、年降水量、海拔、SOC、CEC 等 14 项卫星遥感/地理空间数据" },
      { label: "阈值标准库", desc: "GB 15618（农用地）与 GB 36600（建设用地）分用途阈值，含 pH 路由和质地条件" },
      { label: "修复后用途", desc: "生产用地或生态用地，决定使用哪一轨的阈值上下文和用途权重" },
    ],
    processes: [
      { label: "明确障碍判定 B", desc: "按上限型/下限型/区间型/等级型四种规则判定因子是否构成障碍（B=1/0），无阈值的因子不判定、标记建议补测" },
      { label: "规则严重度 R", desc: "R = min(1, log(1+x/U) / log(1+c))，对数截尾防极端值劫持，区间型和下限型有专属公式" },
      { label: "用途权重 W", desc: "锚定课题二 AHP + 熵值组合赋权，非 RF 内生权重；生产轨侧重食用安全，生态轨侧重生态毒性" },
      { label: "模型贡献度 M", desc: "TreeSHAP 计算各因子对障碍指数 OI 的加性贡献，归一化为模型全局贡献份额；SHAP 是模型贡献，不是法规判定，也不代表障碍高度" },
      { label: "KOS 综合评分", desc: "KOS = B × (0.30R + 0.25W + 0.15M + 0.20S + 0.10E)，仅在确认是障碍(B=1)时计算，含稳定性S和证据强度E(ABCD四级)" },
      { label: "LLM 报告润色", desc: "AI 将技术性结论转化为通俗语言，供非技术背景人员阅读；经事实校验但仍有降级回退机制，最终以原始检测数据为准" },
    ],
    outputs: [
      { label: "关键障碍因子 Top-N", desc: "按 KOS 综合得分降序排列，仅 A/B 级证据因子进入正式排名" },
      { label: "模型验证指标", desc: "Spearman 秩相关系数（交叉验证），反映模型排序可靠性" },
      { label: "KOS 五分量分解", desc: "每个障碍因子的 R/W/M/S/E 五个分量数值及贡献占比" },
      { label: "补测建议清单", desc: "无阈值或证据不足(C/D级)的因子，提示需要补检的项目和原因" },
      { label: "诊断报告 (PDF)", desc: "含场地背景、KOS 排序、五分量分解、模型指标、补测清单、追溯审计" },
    ],
    references: [
  ...
  reconstruction_eval: {
    ...
    inputs: [
      { label: "障碍因子诊断结果", desc: "KOS 诊断输出的关键障碍因子列表及 KOS 综合得分" },
