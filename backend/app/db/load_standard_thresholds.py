...
#
# 辛特助 2026-06-25 补录(裴总 deep-research 授权: "如果有添加进去…备注情况")。
...
#   HIGH   — 辛特助矢量PDF多模态识别(非扫描OCR, 幻觉风险低),
#            与乙苯7.2/28(裴总亲口纠正web搜28错)同源交叉印证;
#   MEDIUM — 权威CSV "裴总核对原文,high"(辛特助未亲识别该行, 采纳裴总核对值, 待最终复核);
...
_GB36600_ORGANIC: dict[str, tuple] = {
    ...
    "乙苯": (7.2, 28, 72, 280, "表1#30", "high", "裴总亲口纠正(web搜28错), 已交叉印证"),
    ...
    # ── MEDIUM(裴总核对原文权威CSV, 辛特助 2026-06-25 未亲识别, 待最终复核) ──
    "多氯联苯": (0.2, 2.0, None, None, "表1末", "medium",
                 "裴总核对原文(权威CSV); 辛特助未亲识别, cat2语义待裴总最终复核"),
    "石油烃": (826, 4500, None, None, "附录B", "medium",
              "裴总核对原文(权威CSV); 石油烃(C10-C40); 辛特助未亲识别"),
    "DDT类": (1.0, 4.0, None, None, "表1", "medium",
              "裴总核对原文(权威CSV); p,p'-DDT总量; 辛特助未亲识别"),
    "六六六": (0.4, 2.0, None, None, "表1", "medium",
              "裴总核对原文(权威CSV); HCH总量; 辛特助未亲识别"),
        ...
        for (land, scenario), screening, control in zip(
            ...
            rows.append({
                ...
                "notes": (f"[{conf.upper()}] {table_ref}; "
                          + (note or "辛特助2026-06-25矢量PDF多模态识别(与乙苯7.2/28同源交叉印证)")),
            })
    return rows
